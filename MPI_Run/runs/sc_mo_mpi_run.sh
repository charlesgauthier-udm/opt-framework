...
cp -v $MPI_Run/sc_newscores/main_sc_mo.py ./
cp -v $MPI_Run/fenv.py ./
cp -v $MPI_Run/fscore.py ./
cp -v $MPI_Run/sc_newscores/algo_sc_mo.py ./
cp -v $MPI_Run/sc_newscores/loss_model_global_mpi_sc_mo.py ./
...
mpiexec -n 32 python3 main_sc_mo.py --prior=$MPI_Run/prior_classic_scenario3.txt --loss=$MPI_Run/sc_newscores/loss_model_global_mpi_sc_mo.py --algo=tpe --iter=100


cp -v ./sc_mo_opt_out_* $MPI_Run/outputs
