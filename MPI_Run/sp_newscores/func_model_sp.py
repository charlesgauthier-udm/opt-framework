...
def model(param,ncell,iccp2,ignd,litrmass,soilcmas,tf,delt,ndays, thice, thpor, thliq, psisat, tbar, b,
    ...
    apfts = np.array([[param['bsratelt']*0.4453, param['bsratelt']*0.5986, param['bsratelt']*0.6339,
                       param['bsratelt']*0.7576, param['bsratelt']*0.6957,0.6000,0.6, param['bsratelt']*0.5260, param['bsratelt']*0.5260,
                       0.5605, 0.5605],
         [param['bsratesc']*0.0260, param['bsratesc']*0.0260, param['bsratesc']*0.0208,
          param['bsratesc']*0.0208, param['bsratesc']*0.0208, 0.0350, 0.0350,
          param['bsratesc']*0.0125, param['bsratesc']*0.0125, 0.02258, 0.02258],
         [param['humicfac']*0.42, param['humicfac']*0.42, param['humicfac']*0.53,
          param['humicfac']*0.48, param['humicfac']*0.48, 0.10, 0.10,
          param['humicfac']*0.42, param['humicfac']*0.42, 0.45, 0.45]])
    threshold = 0.001  # TODO decide of a acceptable threshold
    ...
    biodiffus = 2.73972e-7 #param['biodiffus'] # Diffusivity coefficient for bioturbation [m^2/day]
    cryodiffus = 1.36986e-6#param['cryodiffus'] # Diffusivity coefficient for cryoturbation [m^2/day]
    tanha = param['tanha'] # Constant a for tanh formulation of respiration Q10 determination
    tanhb = 0.56 #param['tanhb'] # Constant b for tanh formulation of respiration Q10 determination
    tanhc = 0.075 #param['tanhc'] # Constant c for tanh formulation of respiration Q10 determination
    tanhd = 46 #param['tanhd'] # Constant d for tanh formulation of respiration Q10 determination
    r_depthredu = 10 #param['r_depthredu']
    tcrit = -1.0 #param['t_crit']
    frozered = 0.1#param['frozered']
    mois_a = 10000 #param['moisa']
    mois_b = 6 #param['moisb']
    mois_c = 4 #param['moisc']
